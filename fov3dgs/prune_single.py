...
def export_eval_images(gaussians, base_path, iteration=0, scene=None, pipe=None, bg=None):
    # test hvs loss in training set
    if args.monitor_val:
        view_stacks = scene.getTestCameras().copy()
    else:
    ...
    ssim_loss_sum = 0.0
    idx = 0
    folder = os.path.join(base_path,f"{iteration}")
    ...
    for viewpoint_cam in view_stacks:
        with torch.no_grad():
            render_pkg = render(viewpoint_cam, gaussians, pipe, bg, cuda_type="pcheck_obb")
            image = render_pkg["render"]
            loaded_image_np = np.transpose(image.cpu().detach().numpy(), (1, 2, 0)) * 255

            # Convert back to PIL Image (if needed)
            loaded_image = Image.fromarray(loaded_image_np.astype(np.uint8))
            loaded_image.save(os.path.join(folder, f"{idx}.jpg"))
            idx+=1
        ...
        with torch.no_grad():
            ...
            if (iteration % prune_interval == 1) and iteration < args.pruning_iters:
                ...
                if pass_test:
                    ...
                    # perform metric pruning
                    if iteration == 1:
                        print("increase iteration prune")
                        for i in range(per_prune_iterative_times + 100):
                            new_gaussians = metric_pruning(gaussians, prune_ratio=prune_ratio, scene=scene, pipe=pipe, bg=bg)
                            tested_ssim = test_ssim_loss(gaussians, scene=scene, pipe=pipe, bg=bg)
                            tested_psnr = test_psnr_loss(gaussians, scene=scene, pipe=pipe, bg=bg)
                            psnr_pass = tested_psnr >= args.target_psnr
                            ssim_pass = tested_ssim >= args.target_ssim
                            pass_test = psnr_pass & ssim_pass
                            log_message("Testing SSIM loss, SSIM Loss in training/test set: {}".format(tested_ssim), log_path)
                            log_message("Testing PSNR loss, PSNR Loss in training/test set: {}".format(tested_psnr), log_path)
                            if not pass_test:
                                log_message(f"Pruning ends at iter {i}, ", log_path)
                                break
                    ...
                    save_path = scene.get_save_path(iteration, "_after_prune")
                    gaussians.save_ply(save_path)
                    export_eval_images(gaussians, base_path=scene.get_image_save_path(), iteration=iteration, scene=scene, pipe=pipe, bg=bg)
